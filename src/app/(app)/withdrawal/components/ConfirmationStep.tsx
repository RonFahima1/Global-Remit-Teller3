...
export function ConfirmationStep({ client, withdrawalDetails, onComplete }: ConfirmationStepProps) {
  ...
  const handleCopyReceipt = () => {
    const receiptText = `
      ...
      Date: ${withdrawalDetails.timestamp.toLocaleString()}
      
      Client: ${client.firstName} ${client.lastName}
      Client ID: ${client.id}
      
      Amount: ${formatCurrency(withdrawalDetails.amount, withdrawalDetails.currency)}
      
      Note: ${withdrawalDetails.note || 'N/A'}
      ...
      <CardContent className="pt-6 space-y-6">
        ...
        <div className="bg-muted/50 border rounded-lg p-4 space-y-4">
          <div className="text-center pb-2 border-b">
            <h3 className="font-bold text-lg">WITHDRAWAL RECEIPT</h3>
            <p className="text-sm text-muted-foreground">
              {withdrawalDetails.timestamp.toLocaleString()}
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <p className="font-medium">{withdrawalDetails.transactionId}</p>
              </div>
              <div>
            ...
            <div className="pt-2 border-t">
              ...
              <p className="font-bold text-xl">{formatCurrency(withdrawalDetails.amount, withdrawalDetails.currency)}</p>
            </div>
        ...
        <div className="bg-green-50 dark:bg-green-950/20 p-4 rounded-lg">
          ...
          <p className="text-center font-bold text-lg">
            {formatCurrency(
              (client.balances?.find(b => b.currency === withdrawalDetails.currency)?.amount || 0) - withdrawalDetails.amount,
